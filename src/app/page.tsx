              ...
              <table className="w-full text-xs min-w-[480px]">
                ...
                <tbody className="divide-y divide-gray-100">
                  {lineas.map((l, i) => {
                    ...
                    return (
                      <tr key={l.nombre} className={`cursor-pointer transition-colors hover:bg-blue-50 ${i % 2 === 0 ? "bg-white" : "bg-gray-50/70"}`}>
                        ...
                        <td className="px-2 py-2 text-center text-xs font-bold text-gray-900 tabular-nums">{fmt(l.primaNeta)}</td>
                        <td className="px-2 py-2 text-center text-xs font-bold text-gray-800 tabular-nums">{fmt(l.anioAnterior)}</td>
                        <td className="px-2 py-2 text-center text-xs font-bold text-gray-800 tabular-nums">{fmt(l.presupuesto)}</td>
                        <td className={`px-2 py-2 text-center text-xs tabular-nums ${diffColor} ${diff < 0 ? 'font-bold' : 'font-bold'}`}>
                  ...
                  <tr className="font-bold border-t-2 border-gray-300 bg-[#041224]">
                    <td className="px-2 py-2 text-xs font-bold text-white">Total</td>
                    <td className="px-2 py-2 text-center text-xs font-bold tabular-nums text-white">{fmt(total)}</td>
                    <td className="px-2 py-2 text-center text-xs font-bold tabular-nums text-white">{fmt(totalAA)}</td>
                    <td className="px-2 py-2 text-center text-xs font-bold tabular-nums text-gray-400">{fmt(totalPpto)}</td>
                    <td className="px-2 py-2 text-center text-xs font-bold tabular-nums" style={{ color: (total - totalPpto) < 0 ? '#E62800' : '#059669' }}>
